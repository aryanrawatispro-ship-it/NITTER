...
class TwitterAPIioScraper:
    ...
    async def scrape_community_tweets(self, community_id: str, max_tweets: int = None) -> List[Dict]:
        ...
        if max_tweets is None:
            logger.info(f"Scraping ALL tweets from community via TwitterAPI.io: {community_id}")
        else:
            ...
            async with httpx.AsyncClient() as client:
                while True:
                    ...
                    params = {
                        ...
                        "expansions": "author_id"  # Get user data
                    }
                    ...
                    for tweet in tweets_list:
                        ...
                        # Construct tweet URL
                        tweet_url = f"https://twitter.com/{username}/status/{tweet_id}" if username else None
                        ...
                        tweet_data = {
                            ...
                            'text': tweet.get('text', ''),
                            'html_text': tweet.get('text', ''),
                            'tweet_url': tweet_url,  # Add direct link to tweet
                            'posted_at': datetime.fromisoformat(tweet.get('created_at', '').replace('Z', '+00:00')) if tweet.get('created_at') else datetime.utcnow(),
                            'likes_count': metrics.get('like_count', 0),
                            'retweets_count': metrics.get('retweet_count', 0),
                            'replies_count': metrics.get('reply_count', 0),
                    ...
                    # Check if we've reached the max_tweets limit
                    ...
                    if not pagination_token:
                        logger.info("No more tweets to fetch (no pagination token)")
                        break

                    # Continue to next page
                    logger.info(f"Fetched {len(all_tweets)} tweets so far, continuing...")

                logger.info(f"Successfully scraped {len(all_tweets)} tweets from community via TwitterAPI.io")
                return all_tweets
