"""
Timeline scraper for extracting tweets from a user's timeline on Nitter.
"""
...
class TimelineScraper(BaseScraper):
    ...
    async def scrape_timeline(self, username: str, max_tweets: int = 100) -> List[Dict]:
        ...
        timeline_url = f"{instance_url}/{username}"

        logger.info(f"Scraping timeline: {username} (max {max_tweets} tweets)")

        success = await self.navigate_to_url(timeline_url)
        if not success:
            logger.error(f"Failed to navigate to timeline: {username}")
            return []
        ...
        # Extract tweets
        tweets = await self._extract_tweets(username, max_tweets)

        logger.info(f"Successfully scraped {len(tweets)} tweets from {username}")
        return tweets

    async def _extract_tweets(self, username: str, max_tweets: int) -> List[Dict]:
        """Extract tweet data from the timeline."""
    ...
    async def _extract_tweet_data(self, element, username: str) -> Optional[Dict]:
        ...
        try:
            ...
            if time_element:
                ...
                if time_title:
                    try:
                        # Parse format like "Feb 19, 2024 · 10:30 AM UTC"
            ...
            # Extract reply-to info if it's a reply
    ...
    def _parse_count(self, text: str) -> int:
        """Parse count values (handles K, M, B suffixes)."""
    ...
    def _extract_urls(self, html: str) -> List[str]:
        """Extract URLs from tweet HTML."""
        urls = re.findall(r'href="([^"]+)"', html)
        # Filter out internal Nitter links
